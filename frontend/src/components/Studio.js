...
const Studio = () => {
  ...
  const loadConversationHistory = async (sessionIdToLoad, toolType) => {
    try {
      const backendUrl = process.env.REACT_APP_BACKEND_URL;
      const endpoint = toolType === 'nanobanana' ? 'nanobanana' : toolType === 'google-veo' ? 'google-veo' : toolType === 'sora2' ? 'sora2' : toolType === 'image-upscaler' ? 'image-upscaler' : toolType === 'flux-kontext' ? 'flux-kontext' : toolType === 'kling' ? 'kling' : 'chatgpt5';
      ...
      <div className="pt-20 md:pt-20 flex h-screen gap-4 p-4" style={{ paddingTop: isMobile ? '6.5rem' : '5rem' }}>
        ...
        <div className="flex-1 flex flex-col h-full relative">
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-4 min-h-0 max-h-full pb-32">
            {/* Historique conversationnel pour NanoBanana, ChatGPT-5, Google Veo, SORA 2, Image Upscaler, Flux Kontext et Kling AI ou zone de résultat pour les autres */}
            {(isNanoBanana || isChatGPT5 || isGoogleVeo || isSora2 || isImageUpscaler || isFluxKontext || isKling) ? (
              <>
          ...
          <div className={`bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-4 shadow-2xl ${
            ...
            {isMobile ? (
              ...
              <div className="flex flex-col gap-3">
                ...
                {/* Options et upload d'images pour Kling AI v2.1 - Desktop */}
                {isKling && (
                  <>
                    {/* Upload des images start et end */}
                    <div className="bg-purple-500/10 border border-purple-400/30 rounded-lg p-4 mb-3 space-y-3">
                      <p className="text-sm font-medium text-gray-200">Images (start obligatoire)</p>
                      ...
                      <div className="space-y-2">
                        ...
                        {klingStartImage && (
                          <img src={klingStartImage.dataUrl} alt="Start" className="w-full max-w-md rounded-lg border border-purple-400/30" />
                        )}
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {klingEndImage && (
                          <img src={klingEndImage.dataUrl} alt="End" className="w-full max-w-md rounded-lg border border-purple-400/30" />
                        )}
                      </div>
                    </div>
