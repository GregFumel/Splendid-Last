...
const Studio = () => {
  ...
  const loadConversationHistory = async (sessionIdToLoad, toolType) => {
    try {
      const backendUrl = process.env.REACT_APP_BACKEND_URL;
      const endpoint = toolType === 'nanobanana' ? 'nanobanana' : toolType === 'google-veo' ? 'google-veo' : toolType === 'sora2' ? 'sora2' : toolType === 'image-upscaler' ? 'image-upscaler' : toolType === 'flux-kontext' ? 'flux-kontext' : toolType === 'kling' ? 'kling' : toolType === 'seedream' ? 'seedream' : toolType === 'grok' ? 'grok' : 'chatgpt5';
  ...
  const handleGenerate = async () => {
    ...
    if ((isNanoBanana || isChatGPT5 || isGoogleVeo || isSora2 || isImageUpscaler || isFluxKontext || isKling || isSeedream || isGrok) && sessionId) {
      ...
      <div className="pt-20 md:pt-20 flex h-screen gap-4 p-4" style={{ paddingTop: isMobile ? '6.5rem' : '5rem' }}>
        ...
        <div className="flex-1 flex flex-col h-full relative">
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-4 min-h-0 max-h-full pb-32">
            {/* Historique conversationnel pour NanoBanana, ChatGPT-5, Google Veo, SORA 2, Image Upscaler, Flux Kontext, Kling AI, Seedream 4 et Grok ou zone de résultat pour les autres */}
            {(isNanoBanana || isChatGPT5 || isGoogleVeo || isSora2 || isImageUpscaler || isFluxKontext || isKling || isSeedream || isGrok) ? (
              <>
                    ...
                    {conversationHistory.map((message, index) => (
                      <div key={message.id || index} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'} mb-4`}>
                        <div className={`max-w-xs lg:max-w-md xl:max-w-lg rounded-xl p-4 ${
                          ...
                          {/* Affichage des images générées pour NanoBanana, Flux Kontext Pro et Seedream 4 */}
                          {(isNanoBanana || isFluxKontext || isSeedream) && message.role === 'assistant' && message.image_urls && message.image_urls.length > 0 && (
                            <div className="mt-3 space-y-2">
                              {message.image_urls.map((imageUrl, imgIndex) => (
            ...
            {isMobile ? (
              ...
              <div className="flex flex-col gap-3">
                ...
                <button
                  ...
                  disabled={(isImageUpscaler ? !uploadedImage : isKling ? (!klingStartImage || !prompt.trim()) : !prompt.trim()) || isGenerating}
