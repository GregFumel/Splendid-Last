...
async def generate_video_with_veo(request: GenerateVideoRequest):
    ...
    try:
        ...
        try:
            ...
            # Ajouter l'image de départ si présente (convertir data URL en URL publique)
            # Le paramètre 'image' est pour l'image-to-video (animer une image de départ)
            ...
            # Ajouter des reference images si présentes (max 3 pour R2V)
...
async def generate_video_with_sora2(request: GenerateVideoSora2Request):
    ...
    try:
        ...
        replicate_token = os.environ.get('REPLICATE_API_TOKEN')
        openai_key = os.environ.get('OPENAI_API_KEY')  # Optionnel - si fourni, sera facturé directement par OpenAI
